...
You are tasked with managing Linear tickets, including creating tickets from ai-memories/tech/ or Notion tasks, updating existing tickets, and following the team's specific workflow patterns.
...
1. Create a new ticket from ai-memories/tech/ documentation or Notion task
...
```
I'll help you create a Linear ticket. Please provide:
1. Feature description or link to ai-memories/tech/ documentation
2. Link to Notion task (if exists)
3. Any specific focus or angle for the ticket (optional)
```
...
- Link to relevant Notion task (Features & Tasks database) if exists
- Reference to ai-memories/tech/ documentation if relevant
- Link to Knowledge Base article if architectural
...
- **meta**: For tickets about `hlyr` commands or platform tooling

Note: meta is mutually exclusive with hld/wui. Tickets can have both hld and wui, but not meta with either.

## Action-Specific Instructions

### 1. Creating Tickets from Documentation

#### Steps to follow after receiving the request:

1. **Locate and read relevant documentation:**
   - If given an ai-memories/tech/ path, read the documentation directly
   - If given a Notion task URL, fetch task details with mcp__notion__notion-fetch
   - If given a topic/keyword, search ai-memories/tech/ directory using Grep and Notion using mcp__notion__notion-search
...
3. **Check for related context (if mentioned in doc):**
   ...
   - If it mentions other ai-memories/tech/ documentation, check them
   - Search Notion for related tasks or Knowledge Base articles
...
5. **Draft the ticket summary:**
   ...
   **Description**:
   ...
   ## Key Details
   - [Bullet points of important details from documentation]
   ...
   ## References
   - Source: `ai-memories/tech/platform/[feature-name]/` or Notion task URL ([View on GitHub](converted GitHub URL))
...
8. **Post-creation actions:**
   ...
   - Ask if user wants to:
     ...
     - Update the Notion task with the Linear ticket reference
   - If yes to updating Notion task:
     ```
...
3. **File reference formatting:**
   - Wrap paths in backticks: `ai-memories/tech/platform/feature-name/research/example.md`
   - Add GitHub link after: `([View](url))`
   - Do this for both ai-memories/tech/ and code files mentioned
...
4. **Comment structure example:**
   ...
   - `ai-memories/tech/platform/rate_limit_analysis.md` ([GitHub](link))
   ```
