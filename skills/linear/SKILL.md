...
You are tasked with managing Linear tickets, including creating tickets from ai_docs or Notion tasks, updating existing tickets, and following the team's specific workflow patterns.
...
1. Create a new ticket from ai_docs documentation or Notion task
...
1. Feature description or link to ai_docs documentation
...
- Reference to ai_docs/ documentation if relevant
...
1. **Locate and read relevant documentation:**
   - If given an ai_docs path, read the documentation directly
   - If given a Notion task URL, fetch task details with mcp__notion__notion-fetch
   - If given a topic/keyword, search ai_docs/ directory using Grep and Notion using mcp__notion__notion-search
...
3. **Check for related context (if mentioned in doc):**
   ...
   - If it mentions other ai_docs documentation, check them
...
5. **Draft the ticket summary:**
   ...
   **Description**:
   ...
   ## References
   - Source: `ai_docs/[feature-name]/` or Notion task URL ([View on GitHub](converted GitHub URL))
...
3. **File reference formatting:**
   - Wrap paths in backticks: `ai_docs/feature-name/research/example.md`
   - Add GitHub link after: `([View](url))`
   - Do this for both ai_docs/ and code files mentioned
...
4. **Comment structure example:**
   ...
   - `ai_docs/rate_limit_analysis.md` ([GitHub](link))
   ```
